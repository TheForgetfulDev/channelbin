...
`refresh_source` does `resp.content` and hands the raw bytes to `import_source`.
...
Decompression lives in `import_source` rather than `refresh_source` so the Xtream
...
from app.accounts import import_source, _maybe_gunzip, refresh_source  # noqa: E402
...
class GzippedImportTests(unittest.TestCase):
    """End to end through import_source against a real temp DB."""
    ...
    def test_gzipped_xmltv_imports_the_same_entries_as_plain(self):
        ...
        n_gz, reason = import_source(make_epg_source(self.account), gzip.compress(raw), epg_days=3)
        self.assertIsNone(reason)
        self.assertEqual(n_gz, 6)
        self.assertEqual(EPGEntry.query.count(), 6)

    def test_gzipped_fetch_throughrefresh_source(self):
        ...
        with mock.patch('app.accounts.requests.get', return_value=resp):
            n, reason = refresh_source(make_epg_source(self.account, url=self.account.epg_url), 30, 3)
        self.assertIsNone(reason)
    ...
    def test_collapse_guard_sees_the_decompressed_content(self):
        ...
        db.session.commit()
        raw = _xmltv(['ch1', 'ch2'], per_channel=3)
        n, reason = import_source(make_epg_source(self.account), gzip.compress(raw), epg_days=3)
    ...
    def test_corrupt_gzip_keeps_existing_epg(self):
        ...
        import_source(make_epg_source(self.account), raw, epg_days=3)
        before = EPGEntry.query.count()
        self.assertEqual(before, 2)

        n, reason = import_source(make_epg_source(self.account), gzip.compress(raw)[:20], epg_days=3)
