...
class DerivedReferenceTests(unittest.TestCase):
    ...
    def test_the_derived_reference_ignores_the_lock(self):
        from app.routes.channel_groups import group_detail_rows, _group_detail_job
        ...
        payload = group_detail_rows(grp, _group_detail_job(grp))
    ...
    def test_a_health_check_only_member_never_becomes_the_derived_reference(self):
...
class DetailPageReadsWhatTheRecorderReadsTests(unittest.TestCase):
    ...
    def _group_with_own_check(self, channels):
        ...
        db.session.flush()
        job = OnDemandTestJob(name='Group check', status='COMPLETE', group_id=grp.id)
        db.session.add(job)
        db.session.commit()
        return grp, job
