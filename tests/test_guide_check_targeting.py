...
class RetargetAnnouncementTests(unittest.TestCase):
    """Changing what a scheduled check tests is a behavior change the user must see
    (product principle 1), so it is announced once rather than silently applied. The
    "once" is a fact recorded in a UserPref, never inferred from a side effect of the
    retarget itself (CLAUDE.md "already done is a fact you recorded")."""
    ...
    def _announce(self):
        from app import _announce_guide_check_retarget
        _announce_guide_check_retarget()

    def test_a_changed_count_raises_one_alert_and_one_group_event(self):
        from app import _GUIDE_CHECK_RETARGET_PREF
        from app.database import (Alert, ChannelGroupEvent, UserPref,
                                  GROUP_CHECK_TARGETS_CHANGED)
        # Startup already stamped the pref against an empty database; clear it so this
        # exercises the first startup after the retarget on a populated one.
        db.session.delete(db.session.get(UserPref, _GUIDE_CHECK_RETARGET_PREF))
        # Nothing carries Channel.in_guide, so the old rule probed 0; the group's serving
        # member makes the new one probe 1. That widening is exactly what has to be said.
        best = make_channel(self.acct, name='Best')
        other = make_channel(self.acct, name='Other')
        best.health_score, other.health_score = 90, 10
        make_group(name='FS1', members=[best, other], in_guide=True,
        ...
        self._announce()
        alerts = Alert.query.filter_by(alert_type='HEALTH_CHECK_TARGETS_CHANGED').all()
        self.assertEqual(len(alerts), 1)
        events = ChannelGroupEvent.query.filter_by(
            event_type=GROUP_CHECK_TARGETS_CHANGED).all()
        self.assertEqual(len(events), 1)
        self.assertIn('1', events[0].detail)
        self.assertIsNone(events[0].channel_id, 'a fact about the check, not a membership')

        self._announce()   # a second startup says nothing more
        self.assertEqual(
            Alert.query.filter_by(alert_type='HEALTH_CHECK_TARGETS_CHANGED').count(), 1)
        self.assertIsNotNone(db.session.get(UserPref, _GUIDE_CHECK_RETARGET_PREF))

    def test_an_unchanged_count_is_stamped_but_says_nothing(self):
        from app import _GUIDE_CHECK_RETARGET_PREF
        from app.database import Alert, UserPref
        self.assertIsNotNone(db.session.get(UserPref, _GUIDE_CHECK_RETARGET_PREF),
                             'startup stamps it even when there is nothing to report')
        self.assertEqual(
            Alert.query.filter_by(alert_type='HEALTH_CHECK_TARGETS_CHANGED').count(), 0)
